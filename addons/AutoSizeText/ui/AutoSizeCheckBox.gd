@tool
class_name AutoSizeCheckBox extends CheckBox
...
func _update_label() -> void:
	...
	if Engine.is_editor_hint() and text != "":
		push_warning(
			"The AutoSizeCheckBox '%s' has the text '%s'. Please set the text to 'button_text' instead of the text property."
			 % [name, text]
		)
...
func _sync_label_rect() -> void:
	...
	_label.offset_left = _get_biggest_check_icon_size().x + _get_icon_space()
	_label.offset_top = 0
...
func _prepare_colors() -> void:
	...
	for color_name: String in colors_to_disable:
		_saved_theme_colors[color_name] = get_theme_color(color_name, "CheckBox")
...
func _get_biggest_check_icon_size() -> Vector2:
	...
	for icon_name: String in icons_to_check:
		var themeIcon: Texture2D = get_theme_icon(icon_name, "CheckBox")
		if themeIcon == null:
			continue
...
func _get_icon_space() -> int:
	var margin: int = get_theme_constant("h_separation", "CheckBox")
	var stylebox: StyleBox = get_theme_stylebox("normal", "CheckBox")
